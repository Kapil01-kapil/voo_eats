...
const Upcoming = (props) => {
  ...
  const DeliveredScreen = (order_id, status, remarks) => {
    dispatch(AllOrderActions.Statuschangeby(order_id, status, remarks));
  };
  const _renderData = ({item}) => {
    if (item.orderdetail.payment_status == "COMPLETED") {
      return (
        <View style={{marginBottom: 10}}>
          ...
          <View
            ...
            <Text
              ...
              onPress={DeliveredScreen.bind(this, "dfng", 6, item.id)}>
              Delivered
            </Text>
            <View
              style={{
            ...
            <Text
              style={{color: "white", fontWeight: "bold"}}
              onPress={DeliveredScreen.bind(this, "dfng", 5, item.id)}>
              Cancel
            </Text>
          </View>
        </View>
      );
    } else if (item.orderdetail.payment_status == "PENDING") {
      return (
        <View style={{marginBottom: 10}}>
          <TouchableOpacity
            style={styles.product_card}
